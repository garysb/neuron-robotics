...
from math import ceil, cos, sin, pi, sqrt, tanh
import threading
import time
...
class g_data(thread_template.ThreadTemplate):
	...
	def __init__(self, s_queues, s_connects, s_conds, s_locks, s_sema):
		...
		self.s_queues.create('local')
		self.s_queues.create('local_data')
		# Socket server setup
	...
	def run(self):
		...
		while 1:
			...
			self.s_queues.put('ultrasonic','scan',{'tell':'local','call':'get_values','points':cPickle.dumps(values)})
			self.parse_queue()
	...
	def get_values(self, values=[]):
		...
		"""
		self.values											= cPickle.loads(values)
		self.grid											= [[0.5 for col in range(self.columns)]
																for row in range(self.rows)]
		self.sensorHits()
		self.available										= True
	...
	def sensorHits(self):
		...
		for i in range(len(self.values)):
			...
			self.computeOccupancy(hitx, hity, rebound, i)

	def computeOccupancy(self, hitx, hity, rebound, label = None):
		"""
		...
		steps												= int(round(max(abs(rise/self.cell['mmx']), abs(run/self.cell['mmy']))))
		if steps == 0:
